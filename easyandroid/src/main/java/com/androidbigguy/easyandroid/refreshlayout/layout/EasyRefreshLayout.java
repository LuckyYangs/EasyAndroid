...
import com.androidbigguy.easyandroid.refreshlayout.layout.api.DefaultRefreshInitializer;
...
import static com.androidbigguy.easyandroid.refreshlayout.layout.util.SmartUtil.dp2px;
...
public class EasyRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
    ...
    public EasyRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mHeaderHeight = ta.getDimensionPixelOffset(R.styleable.EasyRefreshLayout_srlHeaderHeight, mHeaderHeight);
        mFooterHeight = ta.getDimensionPixelOffset(R.styleable.EasyRefreshLayout_srlFooterHeight, mFooterHeight);
        mHeaderInsetStart = ta.getDimensionPixelOffset(R.styleable.EasyRefreshLayout_srlHeaderInsetStart, mHeaderInsetStart);
        mFooterInsetStart = ta.getDimensionPixelOffset(R.styleable.EasyRefreshLayout_srlFooterInsetStart, mFooterInsetStart);
        ...
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.EasyRefreshLayout_srlEnableFooterFollowWhenLoadFinished, mEnableFooterFollowWhenNoMoreData);
        mEnableFooterFollowWhenNoMoreData = ta.getBoolean(R.styleable.EasyRefreshLayout_srlEnableFooterFollowWhenNoMoreData, mEnableFooterFollowWhenNoMoreData);
        ...
        mFixedHeaderViewId = ta.getResourceId(R.styleable.EasyRefreshLayout_srlFixedHeaderViewId, mFixedHeaderViewId);
        mFixedFooterViewId = ta.getResourceId(R.styleable.EasyRefreshLayout_srlFixedFooterViewId, mFixedFooterViewId);
        mHeaderTranslationViewId = ta.getResourceId(R.styleable.EasyRefreshLayout_srlHeaderTranslationViewId, mHeaderTranslationViewId);
        mFooterTranslationViewId = ta.getResourceId(R.styleable.EasyRefreshLayout_srlFooterTranslationViewId, mFooterTranslationViewId);
        mEnableNestedScrolling = ta.getBoolean(R.styleable.EasyRefreshLayout_srlEnableNestedScrolling, mEnableNestedScrolling);
        //        mEnableNestedScrollingOnly = ta.getBoolean(R.styleable.EasyRefreshLayout_srlEnableNestedScrollingOnly, mEnableNestedScrollingOnly);
        mNestedChild.setNestedScrollingEnabled(mEnableNestedScrolling);

        mManualLoadMore = mManualLoadMore || ta.hasValue(R.styleable.EasyRefreshLayout_srlEnableLoadMore);
        mManualHeaderTranslationContent = mManualHeaderTranslationContent || ta.hasValue(R.styleable.EasyRefreshLayout_srlEnableHeaderTranslationContent);
        mManualFooterTranslationContent = mManualFooterTranslationContent || ta.hasValue(R.styleable.EasyRefreshLayout_srlEnableFooterTranslationContent);
        //        mManualNestedScrolling = mManualNestedScrolling || ta.hasValue(R.styleable.EasyRefreshLayout_srlEnableNestedScrolling);
    ...
    protected void onDetachedFromWindow() {
        ...
        /*
         * https://github.com/scwang90/EasyRefreshLayout/issues/716
         * 在一些特殊情况下，当触发上拉加载更多后，
                    ...
                    if (mDragDirection == 'v' || (Math.abs(dy) >= mTouchSlop && Math.abs(dx) < Math.abs(dy))) {//滑动允许最大角度为45度
                        ...
                        if (mIsBeingDragged) {
                            ...
                            if (parent instanceof ViewGroup) {
                                //修复问题 https://github.com/scwang90/EasyRefreshLayout/issues/580
                                //noinspection RedundantCast
    ...
    protected void moveSpinnerInfinitely(float spinner) {
        ...
        if (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && spinner < 0
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    if (mLoadMoreListener != null) {
                        mLoadMoreListener.onLoadMore(EasyRefreshLayout.this);
                    } else if (mOnMultiPurposeListener == null) {
                        finishLoadMore(2000);//如果没有任何加载监听器，两秒之后自动关闭
                    }
                    final OnLoadMoreListener listener = mOnMultiPurposeListener;
                    if (listener != null) {
                        listener.onLoadMore(EasyRefreshLayout.this);
                    }
                }
            }, mReboundDuration);
        }
     ...
     * Recommended {@link com.androidbigguy.easyandroid.refreshlayout.layout.listener.SimpleMultiPurposeListener}
     * 设置多功能监听器
     * 建议使用 {@link com.androidbigguy.easyandroid.refreshlayout.layout.listener.SimpleMultiPurposeListener}
     ...
     * Recommended {@link com.androidbigguy.easyandroid.refreshlayout.layout.impl.ScrollBoundaryDeciderAdapter}
     * 设置滚动边界判断器
     * 建议使用 {@link com.androidbigguy.easyandroid.refreshlayout.layout.impl.ScrollBoundaryDeciderAdapter}
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                mTouchSpinner = 0;
                                mIsBeingDragged = false;
                                EasyRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                                EasyRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                EasyRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                                mNestedInProgress = false;
                                mTouchSpinner = 0;
                            }
                        }
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                EasyRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + offsetY + mTouchSlop * 2, 0));
                                EasyRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offsetY, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                EasyRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
